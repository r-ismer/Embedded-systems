...
int col_addr_set_table[4] = {0x0000,0x0000,0x0000,0x00EF}; // invert with page addr
int pag_addr_set_table[4] = {0x0000,0x0000,0x0001,0x003F}; // invert with col addr
//int col_addr_set_table[4] = {0x0000,0x0000,0x0001,0x003F}; // invert with page addr
//int pag_addr_set_table[4] = {0x0000,0x0000,0x0000,0x00EF}; // invert with col addr
...
void lcd_setup(void){
	for(int i = 0; i<CMD_SET_SIZE; i++){
		...
		IOWR_32DIRECT(LT_24_0_BASE, CMD_CMD, cmd->cmd);
		printf("command : %x \n", cmd->cmd);

		IOWR_32DIRECT(LT_24_0_BASE, CMD_NUM, cmd->length);

		if(cmd->length != 0) {
			for(int j = 0; j < (cmd->length); j++){
				IOWR_32DIRECT(LT_24_0_BASE, CMD_DATA, cmd->table[j]);
			}
		}

		IOWR_32DIRECT(LT_24_0_BASE, CMD_WRITE, 1);

		int running = 1;
		do{
			running = IORD_32DIRECT(LT_24_0_BASE, CMD_WRITE);
		}while(running);
	}

	// write frame size to buffer length (78600)
	IOWR_32DIRECT(LT_24_0_BASE, BUFF_LENGTH, FRAME_SIZE);
	return;
}
